...
async def index(request: Request, portfolio_id: str, nav_date: str):
    ...
    for row in data:
        ...
        if str(type_code) == "99" and instrument_id:
            ...
            file_path_upload = os.path.join(UPLOAD_DIR, f"{instrument_id}_{nav_date}.csv")
            file_path_received = os.path.join(RECEIVED_DIR, f"{instrument_id}_{nav_date}.csv")

            if os.path.exists(file_path_upload):
                file_path = file_path_upload
            elif os.path.exists(file_path_received):
                file_path = file_path_received
            else:
                continue
